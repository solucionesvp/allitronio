...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="es">
      <head>
        {/* Precarga del video del Hero — sin esto, el <video> no empieza a
            pedirse hasta que React hidrata y monta el componente; con esto,
            el navegador lo pide en paralelo al resto de la pagina, apenas
            parsea el <head>. Es la primera pieza visual que ve cualquier
            usuario, así que va con la maxima prioridad. */}
        <link rel="preload" as="video" href={HERO.visualVideo} type="video/mp4" fetchPriority="high" />
      </head>
      <body
