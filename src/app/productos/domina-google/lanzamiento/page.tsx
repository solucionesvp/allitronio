...
import { DG_EXCLUDES, DG_IDEAL_CLIENT, DG_REQUIREMENTS } from "@/data/dominaGoogleContent";
import {
  DG_LAUNCH_TIERS,
  DG_LAUNCH_SOLD,
  DG_LAUNCH_MARKETS,
  ...
  return (
    <>
      ...
      <main style={LIGHT_VARS}>
        ...
        {/* ── 4. Casos antes/después (solo con datos reales) ─────────────── */}
        {DG_PROOF_CASES.length > 0 && (
          <section className="w-full bg-[var(--dg-bg-alt)] px-8 py-24 lg:px-16 xl:px-24">
            <div className="mx-auto max-w-[1100px]">
              <motion.h2 {...reveal(0.06)} className={`mb-14 max-w-[640px] ${H2}`} style={{ fontSize: "clamp(1.8rem, 3.6vw, 2.8rem)" }}>
                Así se veían. Así se ven ahora.
              </motion.h2>
              <div className="grid gap-8 sm:grid-cols-2">
                {DG_PROOF_CASES.map((c, i) => (
                  <motion.article key={c.id} {...reveal(0.06 * i)} className="overflow-hidden rounded-sm border border-[var(--dg-line)] bg-[var(--dg-card)]">
                    <div className="grid grid-cols-2 gap-px bg-[var(--dg-line)]">
                      {[
                        { base: c.before, label: "ANTES" },
                        { base: c.after, label: "DESPUÉS" },
                      ].map((side) => (
                        <div key={side.label} className="relative bg-[var(--dg-card)]">
                          <span className="absolute left-2 top-2 z-10 rounded-sm bg-black/70 px-2 py-1 font-display text-[0.5rem] font-bold tracking-[0.2em] text-white">
                            {side.label}
                          </span>
                          <LaunchImage
                            bases={[side.base]}
                            alt={`${side.label} — ${c.business ?? c.niche}`}
                            wrapperClassName="aspect-[4/5] w-full overflow-hidden"
                    ...
                    <div className="flex items-start justify-between gap-4 p-6">
                      <div>
                        ...
                        {c.caption && <p className={`mt-2 ${BODY}`}>{c.caption}</p>}
                      </div>
                      {c.url && (
                        <a
                          href={c.url}
                          ...
                          className="inline-flex shrink-0 items-center gap-1 font-display text-[0.58rem] font-bold tracking-[0.16em] text-[var(--dg-accent-text)]"
                        >
        ...
        {/* ── 8. Para quién es / no incluye (claro) ──────────────────────── */}
        <section className="w-full bg-[var(--dg-bg)] px-8 py-24 lg:px-16 xl:px-24">
          <div className="mx-auto max-w-[1100px]">
            <motion.h2 {...reveal(0.06)} className={`mb-10 max-w-[640px] ${H2}`} style={{ fontSize: "clamp(1.9rem, 3.8vw, 2.8rem)" }}>
            ...
            <div className="flex flex-wrap gap-3">
              {DG_IDEAL_CLIENT.map((item, i) => (
                <motion.span
                  key={item}
                  {...reveal(0.02 * i)}
                  className="rounded-sm border border-[var(--dg-line)] bg-[var(--dg-card)] px-4 py-2 font-body text-[0.8rem] text-[var(--dg-ink)]"
                >
                  {item}
                </motion.span>
              ))}
            ...
            <div className="mt-14 grid gap-12 border-t border-[var(--dg-line)] pt-10 md:grid-cols-2">
