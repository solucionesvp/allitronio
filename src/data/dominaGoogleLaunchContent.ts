...
//   1. Guardar public/assets/products/local/lanzamiento/prueba/caso-01-antes.png
//      y caso-01-despues.png (jpg/png/webp).
//   2. Agregar el objeto abajo. `business: null` = se muestra sin nombre
//      (cliente sin permiso), p. ej. "Clínica dental · Tepic".
//   3. `url` es opcional: solo si el cliente autoriza el enlace.
...
export interface LaunchProofCase {
  ...
  niche: string;
  city: string;
  /** Ruta SIN extensión, ej. `${PRODUCT_LOCAL_LAUNCH.proofDir}/caso-01-antes` */
  before: string;
  after: string;
  /** Una línea de lo que cambió. Solo hechos verificables. */
  caption?: string;
  url?: string;
}
